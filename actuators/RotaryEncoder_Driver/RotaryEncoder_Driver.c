...
static void re_driver_task(void *args)
{
    ...
    while (1) {
        ...
        if (rotary_value == RE_NOTIFY_CW_STEP) {
            ...
            log_info(RE_TAG, "Got CW step - {%u}", re->count.Value);
        }
        ...
        else if (rotary_value == RE_NOTIFY_CW_STEP)
        {
            ...
            log_info(RE_TAG, "Got CC step - {%u}", re->count.Value);
        }

        else
        {
            log_error(RE_TAG, "Unknown rotary state [%u]", rotary_value);
        }
...
#endif
{
    ...
    STATUS_ERROR_CHECK(gpio_config(&pinConfig));
    if (initStatus == STATUS_OK) {
        ...
        log_error(RE_TAG, "Error configuring data pin: 0x%x", initStatus);
    }
    ...
    if (initStatus == STATUS_OK) {
        ...
        if (initStatus != STATUS_OK) {
            log_error(RE_TAG, "Error adding data gpio isr handler: 0x%x", initStatus);
        }
    }
    ...
    if (initStatus == STATUS_OK) {
        ...
        reControl->count.uValue = (reControl->counterMax / 2); /** start the counter in middle of
                                                                  value range **/
        reControl->counterMin = 0;

        if (!task_running) {
            if (xTaskCreate("re_driver_task", re_driver_task, 2048, reControl, 4, &parentTask)
                == pdTRUE) {
                reControl->parentTask = parentTask;
                task_running = true;
            }
        } else {
            log_error(RE_TAG, "Error starting task");
    ...
    if (initStatus == STATUS_OK) {
        timerHandle = xTimerCreate(
            "reDebounceTmr",
            pdMS_TO_TICKS(RE_DEBOUNCE_TIME_MS),
            pdFALSE,
            reControl,
            &debounceExpireCallback);
        if (timerHandle == NULL) {
            log_error(RE_TAG, "Error creating timer");
            initStatus = STATUS_ERR_NO_MEM;
        } else {
    ...
    if (initStatus) {
        log_error(RE_TAG, "Error in initialising Rotary Encoder Driver");
        if (reControl != NULL) {
            heap_caps_free(reControl);
        }
    } else {
        log_info(RE_TAG, "Succesfully intialised Rotary Encoder Driver!");
    }

    return reControl;
}

status_t rotaryEncoder_setSignedCounter(RE_h re, int16_t *set)
{
...
status_t rotaryEncoder_getValue(RE_h re, int16_t *get)
{
    if (re->signedCounter) {
        *get = re->count.Value;
    } else {
...
status_t rotaryEncoder_getLastDir(RE_h re, uint8_t *lastDir)
{
    *lastDir = re->dirLast;
    return STATUS_OK;
}

status_t rotaryEncoder_setCounterMax(RE_h re, uint16_t countMax)
{
    status_t setStatus = STATUS_OK;

    if (re->signedCounter && countMax > INT16_MAX) {
        log_error(RE_TAG, "Error: In signed mode, max value is %d", INT16_MAX);
        setStatus = STATUS_ERR_INVALID_SIZE;
    } else {
...
status_t rotaryEncoder_resetCounter(RE_h re)
{
    if (re->signedCounter) {
        re->count.Value = 0;
    } else {
