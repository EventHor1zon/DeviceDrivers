...
static status_t sx_spi_read_mod_write_mask(SX1276_DEV dev, uint8_t addr, uint8_t data, uint8_t mask, uint8_t *storage) {
    ...
    if(!err) {
        if((reg & mask) != data) {
            ...
            reg |= data;
#ifdef CONFIG_LORASX_SPI_DEBUG
            log_info(LORA_TAG, "Writing value 0x%02x to register %02x", reg, addr);
...
static void test_transmit(SX1276_DEV dev) {
    ...
    status_t err = sx_write_address_byte(dev, SX_LORA_REGADDR_PAYLOAD_LEN, l);

    log_info("Setting tx mode");
...
static void sx1276_driver_task(void *args) {
    ...
    while(1) {
        ...
        //     if(notify) {
        //         log_info(LORA_TAG, "Received Interrupt!!!!!!!!: %lu", notify);

        //         read_interrupts(dev, &reg);
        //         log_info(LORA_TAG, "ISR Register: ");
        //         printBytesOrderExplicit(reg);

        //         log_info(LORA_TAG, "Clearing ISR register");
        //         clear_interrupts(dev);
        //     }
        // }

        log_info(LORA_TAG, "In driver task");
...
status_t sx_setup_lora(SX1276_DEV dev) {
    ...
    err = sx_set_trx_mode(dev, &mode);
    log_info(LORA_TAG, "Setting mode to 'sleep' [%u]", err);
