...
static status_t sample_all_channels(msg_handle_t *handle)
{
    uint8_t channel = 0;
    int16_t adc_val = 0;
...
#endif
{
    status_t istat = STATUS_OK;
    gpio_config_t conf = {0};
    uint8_t channel = 0;

    if (init->data > 39 || init->data < 32) {
        log_error(MSG_TAG, "Error, invalid GPIO for ADC1: Use 32-39 inclusive");
        istat = STATUS_ERR_INVALID_ARG;
    } else {
    ...
    if (istat == STATUS_OK) {
        if (init->adc_width >= ADC_WIDTH_MAX) {
            istat = STATUS_ERR_INVALID_ARG;
            log_error(MSG_TAG, "Error, invalid ADC width: Use 0-3(4 for S2)");
        } else {
    ...
    if (istat == STATUS_OK) {
#ifdef CONFIG_DRIVERS_USE_HEAP
        msg_handle_t *handle = (msg_handle_t *)
            heap_caps_calloc(1, sizeof(msg_handle_t), MALLOC_CAP_8BIT);
        if (handle == NULL) {
            log_error(MSG_TAG, "Error alocating memory for handle!");
            istat = STATUS_ERR_NO_MEM;
        }
#else
    ...
    if (istat == STATUS_OK) {
        ...
        if (xTaskCreate(msg_task, "msg_task", 2048, handle, 3, &th) != pdTRUE) {
            log_error(MSG_TAG, "Error creating task!");
            istat = STATUS_ERR_NO_MEM;
        } else {
            handle->task = th;
        }
    ...
    if (istat == STATUS_OK) {
        TimerHandle_t timer_handle = xTimerCreate(
            ...
            pdTRUE,
            handle,
            msgeq7_timer_callback);
        if (timer_handle == NULL) {
            log_error(MSG_TAG, "Error creating the timer");
            istat = STATUS_ERR_NO_MEM;
        } else {
    ...
    if (istat != STATUS_OK && handle != NULL) {
        heap_caps_free(handle);
    }
#endif

    if (istat == STATUS_OK) {
        log_info(MSG_TAG, "Succesfully started MSG_EQ7 Driver!");
    } else {
        log_info(MSG_TAG, "Failed to started MSG_EQ7 Driver!");
    }
...
status_t msg_update_channels(msg_handle_t *handle)
{
...
status_t msg_get_channel_1(msg_handle_t *handle, int16_t *val)
{
...
status_t msg_get_channel_2(msg_handle_t *handle, int16_t *val)
{
    status_t status = STATUS_OK;
...
status_t msg_get_channel_3(msg_handle_t *handle, int16_t *val)
{
    status_t status = STATUS_OK;
...
status_t msg_get_channel_4(msg_handle_t *handle, int16_t *val)
{
    status_t status = STATUS_OK;
...
status_t msg_get_channel_5(msg_handle_t *handle, int16_t *val)
{
    status_t status = STATUS_OK;
...
status_t msg_get_channel_6(msg_handle_t *handle, int16_t *val)
{
    status_t status = STATUS_OK;
...
status_t msg_get_channel_7(msg_handle_t *handle, int16_t *val)
{
    status_t status = STATUS_OK;
