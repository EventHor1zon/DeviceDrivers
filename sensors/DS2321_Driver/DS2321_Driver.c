...
DS2321_DEV ds2321_init(DS2321_DEV dev, ds2321_init_t *ini)
{
    ...
    if (!err
        && xTaskCreate(ds2321_driver_task, "ds2321_driver_task", 5012, dev, 3, &(dev->t_handle))
               != pdTRUE)
    {
        err = STATUS_ERR_NO_MEM;
...
status_t ds2321_set_twentyfour(DS2321_DEV Dev, bool *state)
{
    ...
    status_t err = i2c_register_read(
        Dev->i2c_bus,
        DS2321_I2C_DEVICE_ADDR,
        DS2321_REGADDR_HOURS,
        1,
        &regval);
    if (!err) {
        if (i && !(regval & (i << 5))) {
...
status_t ds2321_set_hours(DS2321_DEV dev, uint8_t *hours)
{
    status_t status = STATUS_OK;
    uint8_t t = *hours;
    if ((dev->settings.twentyfour_hour && t > 23)
        || ((!(dev->settings.twentyfour_hour)) && (t > 12 || t < 1)))
    {
        status = STATUS_ERR_INVALID_ARG;
    } else {
        uint8_t _s = t % 10;
