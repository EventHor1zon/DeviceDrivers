...
const parameter_t bm_param_map[bm_param_len] = {
    {"Sample Interval",
     1,
     &bm280_getSampleInterval,
     &bm280_setSampleInterval,
     NULL,
     DATATYPE_UINT8,
     7,
     (GET_FLAG | SET_FLAG)},
    {"Filter Setting",
     2,
     &bm280_getFilterSetting,
     &bm280_setFilterSetting,
     NULL,
     DATATYPE_UINT8,
     4,
     (GET_FLAG | SET_FLAG)},
    ...
    {"Temperature",
     4,
     &bm280_getTemperature,
     NULL,
     NULL,
     DATATYPE_FLOAT,
     0,
     (GET_FLAG | STREAM_FLAG)},
    ...
    {"Temperature OSample",
     7,
     &bm280_getTemperatureOS,
     &bm280_setTemperatureOS,
     NULL,
     DATATYPE_UINT8,
     5,
     (GET_FLAG | SET_FLAG)},
...
/** Calibration Functions :  The following calibration fucntions are adapted from the Bosch
 * BME280 Data sheet **/

/* Returns temperature in DegC, resolution is 0.01 DegC. Output value of “5123” equals 51.23
 * DegC.
 */
/* dev->calibrationData.t_fine carries fine temperature as global value int32_t
 * dev->calibrationData.t_fine; */
static int32_t bm280_compensate_T_int32(BM_DEV dev)
{
    if (dev->calibrationAquired) {
        ...
        var1 = ((((adc_T >> 3) - ((int32_t)dev->calibrationData.dig_T1 << 1)))
                * ((int32_t)dev->calibrationData.dig_T2))
               >> 11;
        var2 = (((((adc_T >> 4) - ((int32_t)dev->calibrationData.dig_T1))
                  * ((adc_T >> 4) - ((int32_t)dev->calibrationData.dig_T1)))
                 >> 12)
                * ((int32_t)dev->calibrationData.dig_T3))
               >> 14;
...
static uint32_t bm280_compensate_P_int64(BM_DEV dev)
{
    if (dev->calibrationAquired) {
        ...
        var1 = ((var1 * var1 * (int64_t)dev->calibrationData.dig_P3) >> 8)
               + ((var1 * (int64_t)dev->calibrationData.dig_P2) << 12);
...
static uint32_t bm280_compensate_H_int32(BM_DEV dev)
{
    if (dev->calibrationAquired) {
        ...
        v_x1_u32r =
            (((((adc_H << 14) - (((int32_t)dev->calibrationData.dig_H4) << 20)
                - (((int32_t)dev->calibrationData.dig_H5) * v_x1_u32r))
               + ((int32_t)16384))
              >> 15)
             * (((((((v_x1_u32r * ((int32_t)dev->calibrationData.dig_H6)) >> 10)
                    * (((v_x1_u32r * ((int32_t)dev->calibrationData.dig_H3)) >> 11)
                       + ((int32_t)32768)))
                   >> 10)
                  + ((int32_t)2097152))
                     * ((int32_t)dev->calibrationData.dig_H2)
                 + 8192)
                >> 14));
        v_x1_u32r =
            (v_x1_u32r
             - (((((v_x1_u32r >> 15) * (v_x1_u32r >> 15)) >> 7)
                 * ((int32_t)dev->calibrationData.dig_H1))
                >> 4));
...
static uint16_t bm280_sleepTime(BM_DEV dev)
{
    ...
    switch (timeSetting) {
        case BM_T_STDBY_0_5MS:
            ...
            break;
        case BM_T_STDBY_62_5MS: time = 63; break;
        case BM_T_STDBY_125MS: time = 125; break;
        case BM_T_STDBY_250MS: time = 250; break;
        case BM_T_STDBY_500MS: time = 500; break;
        case BM_T_STDBY_1000MS: time = 1000; break;
#ifdef BME_280
        case BM_T_STDBY_10MS: time = 10; break;
        case BM_T_STDBY_20MS: time = 20; break;
#else
        case BM_T_STDBY_2000MS: time = 2000; break;
        case BM_T_STDBY_4000MS: time = 4000; break;
#endif
        default: time = 1000; break;
    }
...
static status_t bm280_getDeviceStatus(BM_DEV dev)
{
    ...
    trxStatus = i2c_register_read(
        dev->i2cChannel,
        dev->deviceAddress,
        BM_REG_ADDR_DEV_STATUS,
        1,
        &statusBuffer);
...
static status_t bm280_getCalibrationData(BM_DEV dev)
{
    ...
    i2c_register_read(
        dev->i2cChannel,
        dev->deviceAddress,
        (uint8_t)BM_REG_ADDR_DEV_STATUS,
        1,
        &statusReg);
...
#ifdef BME_280
    /** retrieve the calibration data - add +1 to length of bank1 as there's an unused byte (A0,
     * index[24]) in there **/
    uint8_t bufferB[BM_CALIBR_DATA_BANK2_LEN] = {0};
    trxStatus = i2c_register_read(
        dev->i2cChannel,
        dev->deviceAddress,
        (uint8_t)BM_REG_ADDR_DIGT1_LSB,
        BM_CALIBR_DATA_BANK1_LEN,
        buffer);
    trxStatus = i2c_register_read(
        dev->i2cChannel,
        dev->deviceAddress,
        (uint8_t)BM_REG_ADDR_DIGH2_LSB,
        BM_CALIBR_DATA_BANK2_LEN,
        bufferB);
#else
    trxStatus = i2c_register_read(
        dev->i2cChannel,
...
static status_t bm280_InitDeviceSettings(BM_DEV dev)
{
    ...
    i2c_register_write(dev->i2cChannel, dev->deviceAddress, BM_REG_ADDR_CTRL_MEASURE, 1, commands);

    switch (dev->devSettings.sampleMode) {
        case BM_SAMPLE_OFF: break;
        case BM_FORCE_MODE: commands[0] = (uint8_t)BM_FORCE_MODE; break;
        case BM_NORMAL_MODE:
            commands[0] = (uint8_t)BM_NORMAL_MODE;
            commands[1] |= BM_DEFAULT_T_STDBY; /** t_standby = 0.5ms **/
            break;
        default: log_error(BM_DRIVER_TAG, "Error - incorrect sample mode type selected"); break;
    }
    ...
    switch (dev->devSettings.sampleType) {
        case BM_MODE_TEMP: commands[0] |= BM_CTRL_TEMP_BIT; break;
        case BM_MODE_TEMP_PRESSURE: commands[0] |= (BM_CTRL_TEMP_BIT | BM_CTRL_PRESSURE_BIT); break;
#ifdef BME_280
        case BM_MODE_TEMP_HUMIDITY:
            commands[2] = 1;
...
status_t bm280_updateMeasurements(BM_DEV dev)
{
    ...
    if (trxStatus == STATUS_OK) {
        ...
        trxStatus = i2c_register_read(
            dev->i2cChannel,
            dev->deviceAddress,
            BM_REG_ADDR_PRESSURE_MSB,
            (uint16_t)BM_MEASURE_READ_LEN,
            rxBuffer);
    }

    if (trxStatus == STATUS_OK) {
        int32_t regP = (uint32_t)rxBuffer[0] << 16 | (uint32_t)rxBuffer[1] << 8
                       | (uint32_t)rxBuffer[2];
        int32_t regT = (uint32_t)rxBuffer[3] << 16 | (uint32_t)rxBuffer[4] << 8
                       | (uint32_t)rxBuffer[5];
        ...
        switch (dev->devSettings.sampleType) {
            case BM_MODE_TEMP:
                ...
                dev->sensorData.realTemperature = (float)dev->sensorData.calibratedTemperature
                                                  / 100.0;
                if (dev->use_cbuffer) {
...
status_t bm280_setHumidityOS(BM_DEV dev, BM_overSample_t *os)
{
    ...
    if (status == STATUS_OK
        && read_mod_write(dev->i2cChannel, dev->deviceAddress, BM_REG_ADDR_CTRL_HUMID, _os, 0b111)
               == STATUS_OK)
    {
...
status_t bm280_setSampleMode(BM_DEV dev, BM_sampleMode_t *sampleMode)
{
    ...
    if (status == STATUS_OK
        && read_mod_write(dev->i2cChannel, dev->deviceAddress, BM_REG_ADDR_CTRL_MEASURE, sm, 0b11)
               == STATUS_OK)
    {
...
status_t bm280_setSampleInterval(BM_DEV dev, BM_standbyT_t *dT)
{
    ...
    if (_dt >= BM_T_STDBY_END) {
        ...
        i2c_register_read(dev->i2cChannel, dev->deviceAddress, BM_REG_ADDR_CONFIG, 1, &reg)
        == STATUS_OK)
    {
        reg &= 0b00011111;
        reg |= (_dt << 5);
...
void bmCtrlTask(void *args)
{
    ...
    while (1) {
        ...
        if (dev->devSettings.sampleMode == BM_FORCE_MODE) {
            ...
            if (i2c_register_read(
                    dev->i2cChannel,
                    dev->deviceAddress,
                    BM_REG_ADDR_DEV_STATUS,
                    1,
                    &reg)
                & BM_STATUS_MEASURE_MASK)
            {
                while (i2c_register_read(
                           dev->i2cChannel,
                           dev->deviceAddress,
                           BM_REG_ADDR_DEV_STATUS,
                           1,
