    ...
    if (!err) {
        if (ini->drdy_pin > 0) {
            ...
            if (err != STATUS_OK) {
                ...
                gpio_isr_handler_add(ini->drdy_pin, (gpio_isr_t)data_ready_isr, dev)
...
status_t hmc_set_mode(HMC_DEV dev, uint8_t *val)
{
    ...
    if (byte > 1) {
        ...
        err = i2c_register_read(dev->i2c_bus, dev->i2c_address, HMC_REGADDR_CTRL_A, 1, &regval);
        if (!err) {
            log_info("%u\n", regval);
...
status_t hmc_set_interrupt_enabled(HMC_DEV dev, bool *val)
{
    ...
    uint8_t regval = 0;
    bool en = *val;
    err = i2c_register_read(dev->i2c_bus, dev->i2c_address, HMC_REGADDR_CTRL_B, 1, &regval);
    if (!err) {
...
status_t hmc_read_measurements(HMC_DEV dev)
{
    ...
    err = i2c_register_read(dev->i2c_bus, dev->i2c_address, HMC_REGADDR_XDATA_LSB, 6, buffer);
