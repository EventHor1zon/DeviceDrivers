 ...
 *           (allowing people to build their own systems like a proper libraray), but we do have
 *this interrupt servicing task. Maybe go minimalist and implement a fifo auto-read setting or
 *something?
 *
 *
...
const parameter_t max31_param_map[max31_param_length] = {
    ...
    {"Sample Avg",
     3,
     &max31_get_sample_average,
     &max31_set_sample_average,
     NULL,
     DATATYPE_UINT8,
     5,
     (GET_FLAG | SET_FLAG)},
    {"FIFO Rollover",
     4,
     &max31_get_fifo_rollover,
     &max31_set_fifo_rollover,
     NULL,
     DATATYPE_UINT8,
     1,
     (GET_FLAG | SET_FLAG)},
    {"Almost full",
     5,
     &max31_get_almost_full_val,
     &max31_set_almost_full_val,
     NULL,
     DATATYPE_UINT8,
     0x0f,
     (GET_FLAG | SET_FLAG)},
    ...
    {"Sample Rate",
     7,
     &max31_get_spo2_samplerate,
     &max31_set_spo2_samplerate,
     NULL,
     DATATYPE_UINT8,
     7,
     (GET_FLAG | SET_FLAG)},
    ...
    {"Red Amplitude",
     9,
     &max31_get_redledamplitude,
     &max31_set_redledamplitude,
     NULL,
     DATATYPE_UINT8,
     0xFF,
     (GET_FLAG | SET_FLAG)},
    {"IR Amplitude",
     10,
     &max31_get_irledamplitude,
     &max31_set_irledamplitude,
     NULL,
     DATATYPE_UINT8,
     0xFF,
     (GET_FLAG | SET_FLAG)},
    ...
    {"Ambi ovr",
     12,
     &max31_get_ambient_light_invalidates,
     &max31_set_ambient_light_invalidates,
     DATATYPE_BOOL,
     1,
     (GET_FLAG | SET_FLAG)},
};
...
static status_t emit_fifo_almostfull_event(MAX31_h dev)
{
    return event_post(
        dev->event_loop,
        0,
        MAX31_EVENT_FIFO_ALMOST_FULL,
        dev,
        sizeof(void *),
        pdMS_TO_TICKS(CONFIG_SHORTWAIT_MS));
}
...
static status_t emit_fifo_read_done_event(MAX31_h dev)
{
    return event_post(
        dev->event_loop,
        0,
        MAX31_EVENT_FIFO_READ_COMPLETE,
        dev,
        sizeof(void *),
        pdMS_TO_TICKS(CONFIG_SHORTWAIT_MS));
}
...
static status_t read_mod_write(
    uint8_t i2cChannel,
    uint8_t deviceAddr,
    uint8_t regAddr,
    uint8_t clear_bits,
...
{
    uint8_t regval = 0;
    status_t err = i2c_read_address(i2cChannel, deviceAddr, regAddr, 1, &regval);
    if (!err) {
        regval &= ~clear_bits;
        regval |= set_bits;
        err = i2c_write_address(i2cChannel, deviceAddr, regAddr, 1, &regval);
    }
    return err;
}
...
static uint8_t adc_shift_from_pwm(max31_ledpwm_t pwm)
{
    ...
    switch (pwm) {
        ...
        case MAX31_LED_PWM_411:
            /** adc res 18 bits **/
            break;
        default: log_error(MX_TAG, "Error - invalid ledpwm set"); break;
    }
...
static float conversion_factor_from_adcrange(max31_adcrange_t adc_range)
{
    ...
    switch (adc_range) {
        case MAX31_ADC_RNG_2048: conversion_factor = 7.81; break;
        case MAX31_ADC_RNG_4096: conversion_factor = 15.63; break;
        case MAX31_ADC_RNG_8192: conversion_factor = 31.25; break;
        case MAX31_ADC_RNG_16384: conversion_factor = 62.5; break;
        default: log_error(MX_TAG, "Error - invalid adcrange set"); break;
    }
...
static status_t max31_reset_fifo(MAX31_h dev)
{
    ...
    status = i2c_register_write(
        dev->i2c_bus,
        dev->dev_addr,
        (uint8_t)MAX31_REGADDR_FIFO_WRTPTR,
        3,
        &clear_val);
...
static void convert_fifo_data(MAX31_h dev)
{
    ...
    if (dev->dev_settings.device_mode == MAX31_MODE_SPO2_RED_IR
        || dev->dev_settings.device_mode == MAX31_MODE_MULTILED_RIR)
    {
        dual_sample = true;
    }
    ...
    for (counter = 0; counter < dev->bytes_read; counter += 3) {
#ifdef DEBUG_MODE
        log_info(
            "Data Raw[0] %02x Raw[1] %02x Raw[2] %02x ",
            dev->fifo_buffer[counter],
            dev->fifo_buffer[counter + 1],
            dev->fifo_buffer[counter + 2]);
#endif
        raw =
            ((dev->fifo_buffer[counter] << 16) | (dev->fifo_buffer[counter + 1] << 8)
             | (dev->fifo_buffer[counter + 2]));
        ...
        else
        {
            ...
            log_info(
                "Red channel[%u] = %.3f (converted = %.3f)\n",
                increment,
                dev->red_buffer[increment],
                converted);
...
#ifdef CONFIG_ENABLE_MAX31_EVENTS
                    if (dev->use_events && dev->event_mask & MAX31_EVENT_AMBI_OVR) {
...
#ifdef CONFIG_ENABLE_MAX31_EVENTS
                    /** TODO: Events **/
                    if (dev->use_events && dev->event_mask & MAX31_EVENT_FIFO_ALMOST_FULL) {
                    ...
                    if (dev->read_fifo_on_almostfull) {
                        ...
                        if (dev->use_cbuff) {
                            cbuffer_write(
                                dev->cbuff,
                                dev->red_buffer,
                                sizeof(float) * dev->pkts_in_fifo);
                            if (dev->device_mode == MAX31_MODE_MULTILED_RIR
                                || dev->device_mode == MAX31_MODE_SPO2_RED_IR) {
                                cbuffer_write(
                                    dev->cbuff,
                                    dev->ir_buffer,
                                    sizeof(float) * dev->pkts_in_fifo);
    ...
    if (istatus == STATUS_OK) {
        ...
        if (init->intr_pin > 0) {
            ...
            if (istatus == STATUS_OK) {
                ...
                istatus = gpio_isr_handler_add(
                    handle->intr_pin,
                    max31_interrupt_handler,
                    (void *)handle);
                if (istatus != STATUS_OK) {
        ...
        if (istatus == STATUS_OK) {
            ...
            if (xTaskCreate(max31_task, "max31_task", 2056, (void *)handle, 3, &taskhandle)
                != pdTRUE) {
...
{
    uint16_t val = 0;
    status_t err = i2c_register_write(dev->i2c_bus, dev->dev_addr, MAX31_REGADDR_INTR_EN1, 2, &val);
    return err;
}
        ...
        if (!status) {
            ...
            if (v & MAX31_INTR_TEMP_RDY) {
                ...
                status = i2c_register_write(
                    dev->i2c_bus,
                    dev->dev_addr,
                    (uint8_t)MAX31_REGADDR_INTR_EN2,
                    1,
                    &regval);
                /** clear bit **/
            ...
            if (v) {
                ...
                status = i2c_register_write(
                    dev->i2c_bus,
                    dev->dev_addr,
                    (uint8_t)MAX31_REGADDR_INTR_EN1,
                    1,
                    &v);
            }
        } else {
...
status_t max31_set_sample_average(MAX31_h dev, max31_sampleavg_t *val)
{
    ...
    if (v > MAX31_SAMPLE_AVG_32) {
        ...
        status = i2c_register_read(
            dev->i2c_bus,
            dev->dev_addr,
            (uint8_t)MAX31_REGADDR_FIFO_CONFIG,
            1,
            &regval);
...
status_t max31_set_almost_full_val(MAX31_h dev, uint8_t *val)
{
    ...
    if (v > MAX31_ALMOSTFULL_MAX) {
        ...
        status = read_mod_write(dev->i2c_bus, dev->dev_addr, MAX31_REGADDR_FIFO_CONFIG, v, 0b1111);
    }
    if (status == STATUS_OK) {
...
status_t max31_set_shutdown(MAX31_h dev, bool *val)
{
    ...
    status = i2c_register_read(
        dev->i2c_bus,
        dev->dev_addr,
        (uint8_t)MAX31_REGADDR_MODE_CONFIG,
        1,
        &regval);
...
status_t max31_set_mode(MAX31_h dev, max31_mode_t *val)
{
    ...
    if (mode == MAX31_MODE_HEARTRATE_RED || mode == MAX31_MODE_SPO2_RED_IR
...
status_t max31_set_spo2_samplerate(MAX31_h dev, uint8_t *val)
{
    ...
    if (*val > MAX31_SAMPLERATE_3200) {
        ...
        status = i2c_register_read(
            dev->i2c_bus,
            dev->dev_addr,
            (uint8_t)MAX31_REGADDR_SP02_CONFIG,
            1,
            &regval);
        regval |= (*val << 2);
        status = i2c_register_write(
...
status_t max31_set_ledpwm(MAX31_h dev, uint8_t *val)
{
    ...
    if (pwm > MAX31_LED_PWM_411) {
        ...
        status = read_mod_write(dev->i2c_bus, dev->dev_addr, MAX31_REGADDR_SP02_CONFIG, pwm, 0b11);
    }
...
status_t max31_set_redledamplitude(MAX31_h dev, uint8_t *val)
{
    ...
    status = i2c_register_write(
        dev->i2c_bus,
        dev->dev_addr,
        (uint8_t)MAX31_REGADDR_LED1PULSE_AMP,
        1,
        &regval);
...
status_t max31_set_irledamplitude(MAX31_h dev, uint8_t *val)
{
    ...
    status = i2c_register_write(
        dev->i2c_bus,
        dev->dev_addr,
        (uint8_t)MAX31_REGADDR_LED2PULSE_AMP,
        1,
        &regval);
...
status_t max31_get_fifo_ovr(MAX31_h dev, uint8_t *val)
{
    ...
    err = i2c_register_read(
        dev->i2c_bus,
        dev->dev_addr,
        (uint8_t)MAX31_REGADDR_OVRFLW_CNTR,
        1,
        &ovr);
    if (!err) {
        *val = ovr;
...
status_t max31_read_fifo(MAX31_h dev)
{
    ...
    else
    {
        ...
        if (status == STATUS_OK) {
            ...
            avail_samples = wr_ptr_val > rd_ptr_val
                                ? (wr_ptr_val - rd_ptr_val)
                                : (MAX31_FIFO_SAMPLES - rd_ptr_val) + wr_ptr_val;
            ...
            log_info(
                MX_TAG,
                "Write_ptr: %u Rd_Ptr: %u Packets: %u Avail bytes: %u",
                regvals[0],
                regvals[2],
                avail_samples,
                avail_bytes);
        ...
        if (status == STATUS_OK && avail_bytes) {
            ...
            status = i2c_register_read(
                dev->i2c_bus,
                dev->dev_addr,
                (uint8_t)MAX31_REGADDR_FIFO_DATA,
                avail_bytes,
                dev->fifo_buffer);
...
#ifdef CONFIG_ENABLE_MAX31_EVENTS
    if (dev->use_events && dev->event_mask & MAX31_EVENT_FIFO_READ_COMPLETE && status == STATUS_OK)
    {
...
status_t max31_enable_temperature_sensor(MAX31_h dev)
{
    ...
    status = i2c_register_write(
        dev->i2c_bus,
        dev->dev_addr,
        (uint8_t)MAX31_REGADDR_DIETEMP_SAMPLE,
        1,
        &write);
