...
static status_t SetSensorConfig(VL53L0X_DEV Dev, VL53L0X_config_t vl_config);
...
static status_t SetSensorConfig(VL53L0X_DEV Dev, VL53L0X_config_t vl_config) {
    ...
    status_t Status = VL53L0X_SetLimitCheckEnable(
    ...
    switch (vl_config) {
        ...
        // Maybe should convert to helper functions but...
            log_info(VL53_TAG, "Configuring VL53L0X_SENSE_DEFAULT");
        ...
        case VL53L0X_SENSE_HIGH_SPEED:
            log_info(VL53_TAG, "Configuring VL53L0X_SENSE_HIGH_SPEED");
            Status = VL53L0X_SetLimitCheckValue(
                Dev, VL53L0X_CHECKENABLE_SIGNAL_RATE_FINAL_RANGE,
...
static void vl53_driver_task(void *arg) {
    ...
    log_info(VL53_TAG, "Starting VL53L0X Driver Task");
    ...
    while(1) {
        ...
        if(ret != STATUS_OK) {
            log_error(VL53_TAG, "Update Measurements failed!");
        } else {
            log_info(VL53_TAG, "[%u]\tRange (%u)\tMeasureT %u  (ts: %u)", Dev->rangeData.RangeStatus, Dev->rangeData.RangeMilliMeter, Dev->rangeData.MeasurementTimeUsec, Dev->rangeData.TimeStamp);
        }
...
VL53L0X_DEV vl53_init() {
    ...
    if(ret == 0) {
        ...
        if(ret != 0) {
            log_info("GDI Ret was: %u\n", ret);
        } else {
            log_info(VL53_TAG, "Got info -- ");
            log_info(VL53_TAG, "Name: %s", info.Name);
            log_info(VL53_TAG, "Type: %s", info.Type);
            log_info(VL53_TAG, "ProductID: %s", info.ProductId);
            log_info(VL53_TAG, "Product Type: %u", info.ProductType);
            log_info(VL53_TAG, "ProductRev: %u-%u", info.ProductRevisionMajor, info.ProductRevisionMinor);
        }
    }
    if(ret == 0) {
        ret = VL53L0X_StaticInit(Dev);
        log_info(VL53_TAG, "SI Ret was: %i\n", ret);

        ret = VL53L0X_PerformRefSpadManagement(Dev, &spads, &iaspads);
        if(ret != 0) {
            log_info("PRSM Ret was: %i\n", ret);
        } else {
            ret = VL53L0X_PerformRefCalibration(Dev, &Vhv, &phase);
            if(ret != 0) {
                log_info("PRC Ret was: %i\n", ret);
            }
        }
    }
...
status_t vl53_getDeviceMode(VL53L0X_DEV Dev, uint8_t *val) {

    status_t ret = VL53L0X_GetDeviceMode(Dev, val);
    return ret;
}


status_t vl53_setDeviceMode(VL53L0X_DEV Dev, uint8_t *val) {
...
status_t vl53_UpdateMeasurement(VL53L0X_DEV Dev) {
...
status_t vl53_setDevicePwr(VL53L0X_DEV Dev, uint8_t *val) {
...
status_t vl53_getDevicePwr(VL53L0X_DEV Dev, uint8_t *val) {
...
status_t vl53_getDeviceSampleConfig(VL53L0X_DEV Dev, uint8_t *val) {
...
status_t vl53_setDeviceSampleConfig(VL53L0X_DEV Dev, uint8_t *val) {
    ...
    if(conf != VL53L0X_SENSE_DEFAULT &&
        ...
        log_error(VL53_TAG, "Invalid Sample config");
        ret = STATUS_ERR_INVALID_ARG;
    } 
    else {
