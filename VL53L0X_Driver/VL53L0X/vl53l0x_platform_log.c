...
#include <stdio.h>    // slog_info(), vsnlog_info(), log_info()

#ifdef _MSC_VER
#define snlog_info _snlog_info
#endif
...
void trace_print_module_function(uint32_t module, uint32_t level, uint32_t function, const char *format, ...)
{
        ...
        || ((function & _trace_functions) > 0) )
    {
        ...
        vsnlog_info(message, VL53L0X_MAX_STRING_LENGTH_PLT, format, arg_list);
        va_end(arg_list);

        if (_tracefile != NULL)
            flog_info(_tracefile, message);
        else
            log_info(message);
    }
}
