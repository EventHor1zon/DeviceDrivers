...
static void system_task(void *args) {
    ...
    while(1){
        ...
        if(kickstatus == STATUS_ERR_NOT_FOUND ) {
            log_info(SYS_TAG, "Could not reset WDT - Task not subscribed");
        } else if (kickstatus == STATUS_ERR_INVALID_STATE) {
            log_info(SYS_TAG, "Could not reset WDT - WDT not initialised");
        }
...
system_handle_t *system_init() {
    ...
    if(handle == NULL) {
        ...
        if(uptimer == NULL) {
            log_error(SYS_TAG, "Error creating timer");
            init = STATUS_FAIL;
        } else {
        ...
        if(init == STATUS_OK) {
            ...
            if(xTaskCreate(system_task, "system_task", configMINIMAL_STACK_SIZE, (void *)handle, 5, &system_task_handle) != pdTRUE) {
                log_error(SYS_TAG, "Error creating the system task!");
                init = STATUS_ERR_NO_MEM;
            } else {
                /** add the system task to the task_wdt **/
                init = esp_task_wdt_init(SYS_CONFIG_WDTT_TIMEOUT_S, true);
                if(init != STATUS_OK) {
                    log_error(SYS_TAG, "Error initialising task watchdog");
                } else {
                    init = esp_task_wdt_add(system_task_handle);
                    if(init != STATUS_OK) {
                        log_error(SYS_TAG, "Error adding system task to watchdog");
                    }
                }
            }
        }
...
status_t system_set_autosleep_wow(system_handle_t *handle, uint8_t *val) {
    ...
    if(!(bits & WIFI_CONNECTED_BIT)) {
        log_info(SYS_TAG, "Can't Wake on Wifi: Not connected");
        status = STATUS_FAIL;
    } 
    else 
    {
        ...
        if(*val) {
            log_info(SYS_TAG, "Setting auto-sleep with wake-on-wifi");
            status = esp_wifi_set_ps(WIFI_PS_MAX_MODEM);
            if(status != STATUS_OK) {
                log_error(SYS_TAG, "Error setting wifi modem-sleep [%u]", status);
            } 
            else 
            {
                ...
                if(status != STATUS_OK) {
                    log_error(SYS_TAG, "Error configuring power manager [%u]", status);
                }
            }
        } 
        else 
        {
            log_info(SYS_TAG, "Unsetting auto-sleep power save");
            status = esp_wifi_set_ps(WIFI_PS_NONE);
            if(status != STATUS_OK) {
                log_error(SYS_TAG, "Error setting wifi power-save off");
            } 
            else 
            {
...
void dump_system_info()
{
    ...
    esp_read_mac(wmac, STATUS_MAC_WIFI_STA);
    esp_read_mac(btmac, STATUS_MAC_BT);
...
status_t sys_get_total_memory(system_handle_t *handle, uint32_t *val)
{
    status_t status = STATUS_OK;
...
status_t sys_sleep_until_woken(system_handle_t *handle)
{
    status_t status = STATUS_OK;
...
status_t system_enable_ext0_io(system_handle_t *handle) {

    status_t status = STATUS_OK;

    if (sizeof(handle->wakepins) == 0) {
        log_error(SYS_TAG, "Error: No pins assigned to wake!");
        status = STATUS_FAIL;
    } else {
        for(uint8_t i=0; i<(sizeof(handle->wakepins)/sizeof(wake_pin_t)); i++) {
            ...
            log_info(SYS_TAG, "Info: Adding gpio_num %u as wakeup on %u level", handle->wakepins[i].pin, handle->wakepins[i].level);
            if(status != STATUS_OK) {
                log_error(SYS_TAG, "Error adding pin %u as a wakeup source", handle->wakepins[i].pin);
                break;
            }
        }
    }
...
status_t system_enable_ext1_io(system_handle_t *handle) {
    ...
    if (sizeof(handle->wakepins) == 0) {
        ...
        log_info(SYS_TAG, "Info: Adding gpio pins as ext1 wakeup on low");
        status = esp_sleep_enable_ext1_wakeup(mask, STATUS_EXT1_WAKEUP_ALL_LOW);
    }
...
status_t system_register_wakeup_pin(system_handle_t *handle, wake_pin_t *pin_conf) {
    ...
    if(pin_conf->waketype == WAKETYPE_EXT0 && rtc_gpio_is_valid_gpio(pin_conf->pin) > 0) {
        ...
        log_error(SYS_TAG, "Error invalid pin/sleep-mode");
        status = STATUS_FAIL;
    }
