    ...
    uint8_t *txBuffer)
{
    ...
    if (txStatus == STATUS_OK) {
        ...
        txStatus = i2c_master_cmd_begin(
            i2cChannel,
            cmd,
            pdMS_TO_TICKS(GENERIC_I2C_COMMS_TIMEOUT_MS));
        if (txStatus != STATUS_OK) {
...
status_t gcd_i2c_write_address(
    uint8_t i2cChannel,
    uint8_t deviceAddr,
    uint8_t regAddr,
    uint16_t writeLen,
...
status_t gcd_i2c_read_mod_write(
    uint8_t i2cChannel,
    uint8_t deviceAddr,
    uint8_t regAddr,
    uint8_t clear_bits,
    ...
    uint8_t parity)
{
    ...
    err = uart_driver_install(
        bus,
        GCD_CONFIG_UART_RX_BUFF_SIZE,
        GCD_CONFIG_UART_TX_BUFF_SIZE,
        20,
        &uart_queue,
        STATUS_INTR_FLAG_LOWMED);
    if (!err) {
