 ...
 *
 *           For ease of repeatetive programming, this driver only works with spi/i2c and expects
 *valid spi/i2c driver specs. This driver does not start the i2c/spi devices so the user should
 *configure the peripherals themselves.
 *
 *           Thoughts:
...
static status_t mfrc_read_byte_from_address(MFRC_DEV dev, uint8_t address, uint8_t *buffer)
{
    ...
    if (dev->comms_mode == MFRC_SPI_COMMS_MODE) {
        ...
        log_info(
            MFRC_TAG,
            "Sent: %02x to address %02x Received: %02x %02x ",
            trx.tx_data[1] >> 1,
            trx.tx_data[0],
            trx.rx_data[0],
            trx.rx_data[1]);
        ...
        err = i2c_register_read(dev->comms_bus, MFRC_I2C_ADDRESS, address, 1, buffer);
    } else {
...
static status_t mfrc_write_byte_to_address(MFRC_DEV dev, uint8_t address, uint8_t value)
{
    ...
    if (dev->comms_mode == MFRC_SPI_COMMS_MODE) {
        ...
        err = i2c_register_write(dev->comms_bus, MFRC_I2C_ADDRESS, address, 1, &value);
    } else {
        err = STATUS_ERR_INVALID_ARG;
    }
...
static status_t mfrc_read_mod_write_register(
    MFRC_DEV dev,
    uint8_t address,
    uint8_t clear_bits,
    uint8_t set_bits,
    uint8_t *newval)
{
    ...
    if (!err) {
        ...
        else
        {
            log_info(
                MFRC_TAG,
                "Not writing, write byte is equal to read byte (r:%02x, w:%02x)",
                read_byte,
                write_byte);
        }
#endif /** DEBUG_MODE **/
...
{
    uint8_t val = (1 << 7);
    status_t err = mfrc_read_mod_write_register(
        dev,
...

{
    ...
    /** create task **/
    if (!err) {
        if (xTaskCreate(mrfc_driver_task, "mrfc_driver_task", 5012, handle, 3, &handle->task)
            != pdTRUE) {
    ...
    if (!err && handle->irq_en) {
        if (xTaskCreate(mfrc_irq_task, "mrc_irq_task", 5012, handle, 2, &handle->isr_task)
            != pdTRUE) {
...
status_t mfrc_set_command(MFRC_DEV dev, mfrc_cmd_t *cmd)
{
    ...
    if (c != MFRC_COMMAND_IDLE && c != MFRC_COMMAND_MEM && c != MFRC_COMMAND_RAND_ID
        && c != MFRC_COMMAND_CALC_CRC && c != MFRC_COMMAND_TRANSMIT && c != MFRC_COMMAND_NO_CHANGE
        && c != MFRC_COMMAND_RECEIVE && c != MFRC_COMMAND_TRANSCEIVE && c != MFRC_COMMAND_MF_AUTHENT
    ...
    err = mfrc_read_mod_write_register(
        dev,
        MRFC_REGADDR_COMMAND,
        0x0f,
        c,
        &dev->map.command_reg.regval);
...
status_t mfrc_set_antenna_en(MFRC_DEV dev)
{
    ...
    err = mfrc_read_mod_write_register(
        dev,
        MRFC_REGADDR_TX_CONTROL,
        0b11,
        0x03,
        &dev->map.tx_control_reg.regByte);
    return err;
}
...
status_t mfrc_clear_collisions(MFRC_DEV dev)
{
    ...
    err = mfrc_read_mod_write_register(
        dev,
        MRFC_REGADDR_COLLISION_DETECT,
        (1 << 7),
        val,
        &dev->map.collision_reg.regByte);
    return err;
}
...
status_t mfrc_set_rx_uart_sel(MFRC_DEV dev, mfrc_uart_sel_t *sel)
{
    ...
    if (val >= MFRC_UART_SEL_INVALID) {
        ...
        err = mfrc_read_mod_write_register(
            dev,
            MRFC_REGADDR_RX_SELECT,
            (0x03 << 6),
            val,
            &dev->map.rx_sel_reg.regByte);
    }
...
status_t mfrc_get_tmr_prescaler(MFRC_DEV dev, uint16_t *ps)
{
    ...
    *ps =
        (((uint16_t)dev->map.tmode_reg.value.regbits.tprescaler_hi << 8)
...
status_t mfrc_set_fifo_waterlevel(MFRC_DEV dev, uint8_t *wl)
{
    ...
    if (mark > MFRC_FIFO_MAX_WL) {
        ...
        err = mfrc_read_mod_write_register(
            dev,
            MRFC_REGADDR_FIFO_WATERLEVEL,
            MFRC_FIFO_MAX_WL,
            mark,
            &dev->map.fifo_watermark_reg.regByte);
    }
...
status_t mfrc_set_modwidth(MFRC_DEV dev, uint8_t *var)
{
    ...
    err = mfrc_read_mod_write_register(
        dev,
        MRFC_REGADDR_MOD_WIDTH,
        0xFF,
        v,
        &dev->map.modwidth_reg.value.regbyte);
    return err;
}
...
status_t mfrc_get_tmr_reload(MFRC_DEV dev, uint16_t *var)
{
    status_t err = STATUS_OK;
    *var =
        (((uint16_t)dev->map.tmr_reload_reg.value.regbyte << 8)
...
status_t mfrc_get_tmr_ctr(MFRC_DEV dev, uint16_t *var)
{
    status_t err = STATUS_OK;
    uint8_t vals[2] = {0};
    /** TODO: see if we can read both bytes at once - this isn't good practice */
...
status_t mfrc_set_prbs15(MFRC_DEV dev, uint8_t *var)
{
    ...
    v = (v << 5);
    return mfrc_read_mod_write_register(
        dev,
