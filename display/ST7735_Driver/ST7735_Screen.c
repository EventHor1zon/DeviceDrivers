...
static status_t screen_write(screen_handle_t *screen, screen_transaction_t *trx)
{
    ...
    status_t status = spi_transaction(screen->devhandle, &transaction);
    if (status != STATUS_OK) {
        log_error("SPI", "Error in write [%u]", status);
    }
    return status;
}

static status_t screen_writeread(screen_handle_t *screen, screen_transaction_t *trx)
{
    spi_transaction_t transaction = {0};
    ...
    status_t status = spi_transaction(screen->devhandle, &transaction);
    if (status != STATUS_OK) {
        log_error("SPI", "Error in writeread [%u]", status);
    }

    return status;
}

static status_t screen_set_all(screen_handle_t *screen, uint16_t col)
{
...
static status_t screen_init_commands(screen_handle_t *screen)
{
    ...
    trx.send = &byte;
    err = screen_write(screen, &trx);
    log_info(SCRN_TAG, "Resetting Screen [%u]", err);
    vTaskDelay(pdMS_TO_TICKS(500));

    /** exit sleep **/
    if (!err) {
        log_info(SCRN_TAG, "Waking Screen");
        byte = ST_CMD_SLPOUT;
        err = screen_write(screen, &trx);
    }
    ...
    if (!err) {
        log_info(SCRN_TAG, "Frame control... [%u]", err);
        byte = ST_CMD_FRMCTR1;
        err = screen_write(screen, &trx);
    }
    ...
    /** power control **/
    if (!err) {
        log_info(SCRN_TAG, "Power control [%u]", err);
        byte = ST_CMD_PWCTR1;
        trx.mode = ST_CMD_TRX;
        err = screen_write(screen, &trx);
    }
    ...
    /** vcom control **/
    if (!err) {
        log_info(SCRN_TAG, "VCOM control... [%u]", err);
    ...
    /** pixel format **/
    if (!err) {
        log_info(SCRN_TAG, "Pixel format... [%u]", err);
    ...
    log_info(SCRN_TAG, "Screen on... [%u]", err);
    ...
    log_info(SCRN_TAG, "Rotati on... [%u]", err);
...
screen_handle_t *init_screen(st7735_init_t *init)
{
    ...
    if (!err) {
        screen = (screen_handle_t *)
            heap_caps_calloc(1, sizeof(screen_handle_t), MALLOC_CAP_DEFAULT);

        if (screen == NULL) {
            log_error(SCRN_TAG, "Error assigning memory for screen handle");
    ...
    if (!err) {
        ...
        if (err) {
            log_error(SCRN_TAG, "Error setting up gpio! [%u]", err);
        } else {
...
#endif
        log_error(SCRN_TAG, "Error starting ST7735 Driver :(");
    } else {
        log_info(SCRN_TAG, "Succesfully started ST7735 Driver :)");
    }
