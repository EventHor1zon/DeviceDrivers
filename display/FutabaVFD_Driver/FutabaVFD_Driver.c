...
const parameter_t vfd_parameter_map[vfd_parameter_len] = {
    ...
    {"Brightness",
     ...
     NULL,
     DATATYPE_UINT8,
     VFD_CONFIG_MAX_BRIGHTNESS,
     (GET_FLAG | SET_FLAG)},
    {"Update Bright", 18, NULL, NULL, &vfd_update_brightness, DATATYPE_NONE, 0, (ACT_FLAG)},
    {"CGram Page",
     19,
     &vfd_get_current_cgram_page,
     &vfd_set_current_cgram_page,
     NULL,
     DATATYPE_UINT8,
     VFD_NUM_CGRAM_PAGES,
     (GET_FLAG | SET_FLAG)},
    ...
    uint8_t len)
{
    ...
    err = spi_transaction(dev->spi_handle, &trx);
...
static status_t vfd_init_commands(VFD_HANDLE handle)
{
    ...
    if (vfd_write_command_with_data_byte(handle, VFD_CMD_DISPLAY_TIMING, 7) != STATUS_OK
        || vfd_write_command_with_data_byte(
               handle,
               VFD_CMD_DIM,
               (uint8_t)VFD_CONFIG_DEFAULT_DIMMING)
               != STATUS_OK
...
#endif
{
    ...
    if (!err) {
        ioconf.intr_type = GPIO_INTR_DISABLE, ioconf.mode = GPIO_MODE_OUTPUT,
        ioconf.pin_bit_mask = (1 << init->rst_pin), ioconf.pull_down_en = GPIO_PULLDOWN_DISABLE,
...
{
    return vfd_write_command_with_data_byte(
        dev,
        (VFD_CMD_DCRAM_DATA_WRITE | *segment),
        dev->cgram_page);
}
...
status_t vfd_write_segment(VFD_HANDLE dev, uint8_t *segment)
{
    ...
    err = vfd_write_command_with_data_byte(
        dev,
        (VFD_CMD_DCRAM_DATA_WRITE | seg),
        dev->vfd_buff[seg]);
    if (err) {
