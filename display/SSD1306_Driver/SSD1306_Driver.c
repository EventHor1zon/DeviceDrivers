...
static uint16_t scroll_wait_time(ssd1306_handle_t *screen, screen_scroll_dir_t dir)
{
    ...
    switch (screen->scroll_speed) {
        case 0: frames = 5; break;
        case 1: frames = 64; break;
        case 2: frames = 128; break;
        case 3: frames = 255; break;
        case 4: frames = 3; break;
        case 5: frames = 4; break;
        case 6: frames = 25; break;
        case 7: frames = 2; break;
        default: frames = 2; break;
    }
...
status_t ssd1306_set_contrast(ssd1306_handle_t *screen, uint8_t *val)
{
    ...
    err = i2c_register_write(screen->bus, screen->dev_addr, OLED_CONTROL_BYTE_CMD_STREAM, 2, cmds);
    if (!err) {
        screen->contrast = c;
    }
    return err;
...
status_t ssd1306_scroll_screen_dir(ssd1306_handle_t *screen, screen_scroll_dir_t *dir)
{
    ...
    switch (d) {
        ...
        case SCROLL_DIR_RIGHT:
            ...
            write_len = 7;
            break;
        default: err = STATUS_ERR_INVALID_ARG; break;
    }
...
status_t ssd1306_write_current_line(ssd1306_handle_t *screen)
{
    ...
    err = i2c_register_write(screen->bus, screen->dev_addr, OLED_CONTROL_BYTE_CMD_STREAM, 3, cmds);
...
status_t ssd1306_write_text(ssd1306_handle_t *screen)
{
    ...
    stat = i2c_register_write(screen->bus, screen->dev_addr, OLED_CONTROL_BYTE_CMD_STREAM, 3, cmds);
...
ssd1306_handle_t *ssd1306_init(ssd1306_handle_t *handle, ssd1306_init_t *init)
{
    ...
    if (!err) {
        TimerHandle_t timer = xTimerCreate(
            ...
            pdFALSE,
            NULL,
            (TimerCallbackFunction_t)screen_timer_callback);
        if (timer != NULL) {
            handle->timer = timer;
        } else {
