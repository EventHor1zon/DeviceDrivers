...
static status_t set_pixel_brightness(ledtype_t *p_type, void *pixel, uint8_t brightness)
{
    uint8_t *p_arr = pixel;
 ...
 *
 */
static void set_pixel_brt_colour32(
    ledtype_t *p_type,
    void *pixel,
    uint32_t colour,
    uint8_t brightness)
...
void lfx_nightrider(LEDSTRIP_h strip)
{
    ...
    set_pixel_brt_colour32(
        strip->led_type,
        PIXEL_FROM_INDEX(strip, led_pos),
        strip->fx.colour,
        LEDFX_BRIGHTNESS_MAX);
    ...
    if (direction) {
        ...
        if (fade_len && led_pos > 0) {
            ...
            for (int i = 0; (i < fade_len) || (i < led_pos); i++) {
                set_pixel_brt_colour32(
                    strip->led_type,
                    PIXEL_FROM_INDEX(strip, led_pos - i),
                    strip->fx.colour,
                    (int)(LEDFX_BRIGHTNESS_MAX / i + 2));
            }
        }
    } else {
        /* write fading data */
 ...
 */
status_t lfx_set_mode(LEDSTRIP_h strip, ledEffect_t effect)
{
    status_t err = STATUS_OK;

#ifdef DEBUG_MODE
    log_info("FX", "Updating function %u", effect);
#endif
