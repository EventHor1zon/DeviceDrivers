...
static void IRAM_ATTR ws2812_TranslateDataToRMT(
    const void *src,
    rmt_item32_t *dest,
    size_t src_size,
    size_t wanted_num,
...
static status_t ledstrip_rmt_write(LEDSTRIP_h strip)
{
    ...
    transmitStatus = rmt_write_sample(
        strip->channel,
        (uint8_t *)strip->strand_mem_start,
        strip->write_length,
        false);
                ...
                if (xTimerGetPeriod(strip->timer) != strip->fx.frame_time) {
                    ...
                    if (xTimerStop(strip->timer, LEDSTRIP_CONFIG_GENERIC_TIMEOUT) != pdTRUE) {
                        ...
                        xTimerChangePeriod(
                            strip->timer,
                            strip->fx.frame_time,
                            LEDSTRIP_CONFIG_GENERIC_TIMEOUT)
                        != pdTRUE)
                    {
...
status_t ledstrip_driver_init()
{
    ...
    if (!err) {
        if (xTaskCreate(
                ledstrip_driver_task,
                "ledstrip_driver_task",
                LEDSTRIP_CONFIG_TASK_STACK,
                NULL,
                LEDSTRIP_CONFIG_TASK_PRIO,
                &ledstrip_taskhandle)
            != pdTRUE)
        {
...
status_t ledstrip_add_strip(LEDSTRIP_h strip, ledstrip_init_t *init)
{
    ...
    if (!err) {
        ...
        timer = xTimerCreate(
            ...
            pdTRUE,
            (void *)strip,
            (TimerCallbackFunction_t)led_timer_callback);
...
#ifdef DEBUG_MODE
//        log_info("Type: %s %u %u %u\n\n", type->name, type->pixel_bytes, type->brt_bits,
//        (uint32_t)type->init);
#endif
    }
...
status_t ledstrip_set_brightness(LEDSTRIP_h strip, uint8_t *brightness)
{
    ...
    if (strip->led_type->brt_bits == 0) {
        ...
        log_error(
            LS_TAG,
            "Invalid led brightness (max: %u) [%u]",
            ((1 << strip->led_type->brt_bits) - 1),
            err);
    } else {
