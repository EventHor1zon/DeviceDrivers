...
static status_t LSM_waitSampleReady(LSM_DriverHandle_t *dev, uint8_t mask);

/** get the value in the device ID register **/
static status_t LSM_getWhoAmI(LSM_DriverHandle_t *device, uint8_t *whoami);
...
static status_t process_interrupts(LSM_DriverHandle_t *dev, uint32_t sources);
    ...
    for (uint8_t i = 0; i < desc->pkt_elements; i++) {
        switch (desc->pkt_types[i]) {
            case LSM_PKT_ELM_T_ACCEL:
                ...
                // log_info("decoding accel packet index %u input %u : ouput %.2f : factor: %.2f\n",
...
#ifdef DEBUG_MODE
    log_info("Raw Accel Data: %u %u %u\n", sa_x, sa_y, sa_z);
    log_info("Even rawer \n");
    for (uint8_t i = 0; i < 6; i++) {
        log_info("0x%02x ", dev->measurements.rawAccel[i]);
    }
    log_info("\n");
#endif
...
#ifdef DEBUG_MODE
    log_info("Raw Gyro Data: %u %u %u\n", x, y, z);
    log_info("Even rawer \n");
    for (uint8_t i = 0; i < 6; i++) {
        log_info("0x%02x ", dev->measurements.rawGyro[i]);
    }
    log_info("\n");
#endif
...
static int16_t LSM_get_packet_length(LSM_DriverHandle_t *dev)
{
        ...
        && (dev->settings.accelRate > 0 && dev->settings.gyroRate > 0))
    {
        ...
        if (a_mod > g_mod) {
            ...
            log_info("Accel more packets than gyro by factor of %u\n", factor);
        } else {
            dif = g_mod - a_mod;
            factor = (uint8_t)pow(2, (double)dif);
            ...
            log_info("Gyro more packets than accel by factor of %u\n", factor);
        }

        /** dont care about order, this is length **/
        len += ((sizeof(int16_t) * n_axis) * factor);
        log_info("With this in mind, len is now %u\n", len);
    }
    ...
    if (dev->settings.temp_pkt_en) {
        ...
        dev->fifo_pkts.pkt_types[n_elements] = LSM_PKT_ELM_T_GYRO;
        n_elements++;
        log_info("Third packet is temperature : len = %u\n", len);
    }
    ...
    if (dev->settings.pedo_fifo_en) {
        ...
        dev->fifo_pkts.pkt_types[n_elements] = LSM_PKT_ELM_T_GYRO;
        n_elements++;
        log_info("Third packet is step : len = %u\n", len);
    }
...
void LSMDriverTask(void *args)
{
    ...
    while (1) {
        ...
        if (dev->int1En || dev->int2En) {
            ...
            log_info("LSM", "LSM Notify val: %02x", notify_val);
            ...
            if (notify_val) {
                ...
                if (!err) {
                    ...
                    if (dev->status.fifo_thresh) {
                        log_info("LSM", "Got a fifo thresh interrupt");
                    ...
                    if (dev->status.temp_drdy && dev->driver_settings.auto_read_sample_rdy) {
                        log_info("LSM", "Got a temp ready interrupt");
                    ...
                    if ((dev->status.accel_drdy || dev->status.gyro_drdy)
                        ...
                        log_info("LSM", "Got a data ready interrupt");
...
#endif
{
    ...
    if (initData == NULL) {
        log_error("LSM Driver", "Error - NULL ptr to init data");
    ...
    {
        log_error("LSM Driver", "Error - invalid comms channel");
        initStatus = STATUS_ERR_INVALID_ARG;
    }
    ...
    if (initStatus == STATUS_OK) {
        ...
        if (device == NULL) {
            log_error("LSM Driver", "Error assigning mem for the device structure");
        ...
        /** set up interrupts **/
        if (initStatus == STATUS_OK && (initData->int1Pin || initData->int2Pin)) {
            uint32_t gpioMask = 0;
            if (initData->int1Pin) {
            ...
            if (initStatus == STATUS_OK) {
                initStatus = gpio_install_isr_service(STATUS_INTR_FLAG_LEVEL1);
                if (initStatus == STATUS_ERR_INVALID_STATE) {
                    /** driver already initialised, don't freak out */
                    log_info("LSM_Driver", "Cannot install gpio isr service");
                ...
                if (initStatus == STATUS_OK) {
                    ...
                    log_error("LSM Driver", "Error installing ISR service [%u]", initStatus);
                }
            } else {
                log_error("LSM Driver", "Error configuring GPIO pins! [%u]", initStatus);
            }
        }

        if (initStatus == STATUS_OK && initData->use_cbuffer && initData->cbuff != NULL) {
            log_info("LSM_Driver", "Configuring Cbuffer for use!");
        ...
        } else if (initStatus == STATUS_OK && initData->assignFifoBuffer) {
            ...
            if (dmaMem < LSM_FIFO_BUFFER_MEM_LEN) {
                ...
                if (fifoMem != NULL) {
                    ...
                    log_info("LSM_Driver", "FIFO Buffer initialised at %p", fifoMem);
                } else {
                    log_error("LSM", "Error insufficient memory for buffer!");
                    initStatus = STATUS_ERR_NO_MEM;
                }
            }
        }
        ...
        if (initStatus == STATUS_OK) {
            ...
            if (initStatus == STATUS_OK && who != LSM_WHOAMI) {
                log_error(
                    "LSM Driver",
                    "Error - invalid device id! Expected %02x, Recevied: %02x",
                    LSM_WHOAMI,
                    who);
                initStatus = STATUS_ERR_INVALID_RESPONSE;
            } else {
                log_info("LSM_Driver", "Device ID read succesful!");
            }
        }

        if (initStatus == STATUS_OK) {
            /** initialise device settings **/
            log_info("LSM_Driver", "Setting Data rates");
        ...
        if (initStatus == STATUS_OK) {
            ...
            if (xTaskCreate(LSMDriverTask, "LSMDriverTask", 5012, (void *)device, 3, &taskHandle)
                == pdFALSE) {
                log_error("LSM_Driver", "Error creating control task!");
                initStatus = STATUS_ERR_INVALID_RESPONSE;
            }
        ...
        if (initStatus == STATUS_OK) {
            log_info("LSM_Driver", "Succesfully started driver!");
        }
        return device;
    }

    status_t LSM_deInit(LSM_DriverHandle_t * dev)
    {
    ...
    status_t LSM_run_accel_self_test(LSM_DriverHandle_t * dev, bool *passed)
    {
        ...
        if (!(regval & 1)) {
            while (!(regval & 1)) {
                ...
                if (abort > LSM_SELF_TEST_TIMEOUT_N) {
                    log_error("LSM_Driver", "Error: Unable to get data!");
                    *passed = false;
    ...
    status_t LSM_set_auto_read_fifo_full(LSM_DriverHandle_t * dev, bool *en)
    {
        dev->driver_settings.auto_read_fifo_full = *en;
        return STATUS_OK;
    }

    status_t LSM_get_auto_read_fifo_full(LSM_DriverHandle_t * dev, bool *en)
    {
        *en = dev->driver_settings.auto_read_fifo_full;
        return STATUS_OK;
    }

    status_t LSM_set_auto_sample_ready(LSM_DriverHandle_t * dev, bool *en)
    {
        dev->driver_settings.auto_read_sample_rdy = *en;
        return STATUS_OK;
    }

    status_t LSM_get_auto_sample_ready(LSM_DriverHandle_t * dev, bool *en)
    {
        *en = dev->driver_settings.auto_read_sample_rdy;
        return STATUS_OK;
    }

    status_t LSM_set_poll_sample_ready(LSM_DriverHandle_t * dev, bool *en)
    {
    ...
    status_t LSM_get_poll_sample_ready(LSM_DriverHandle_t * dev, bool *en)
    {
        *en = dev->driver_settings.poll_sample_ready;
        return STATUS_OK;
    }

    status_t LSM_set_poll_sample_period_ms(LSM_DriverHandle_t * dev, uint16_t * val)
    {
        status_t err = STATUS_OK;
        uint16_t v = *val;
        ...
        if (v > (uint16_t)LSM_DRIVER_MAX_SAMPLE_PERIOD_MS
            ...
            log_error("LSM Driver", "Error, invalid sample period (min 20, max 100,000 ms)");
    ...
    status_t LSM_get_poll_sample_period_ms(LSM_DriverHandle_t * dev, uint16_t * val)
    {
    ...
    status_t LSM_setOpMode(LSM_DriverHandle_t * dev, LSM_OperatingMode_t * mode)
    {
        status_t status = STATUS_OK;
        uint8_t regVals[2] = {0};
        ...
        log_info("LSM_Driver", "Set opmode to %02x", *mode);

        return status;
    }

    status_t LSM_getOpMode(LSM_DriverHandle_t * dev, LSM_OperatingMode_t * mode)
    {
        status_t status = STATUS_OK;
    ...
    status_t LSM_setAccelODRMode(LSM_DriverHandle_t * dev, LSM_AccelODR_t * m)
    {
        status_t status = STATUS_OK;
        uint8_t accelEn = 0, regVal = 0;
        uint8_t mode = *m;

        log_info("LSM_Driver", "Setting Accel ODR to %u\n", mode);
    ...
    status_t LSM_getAccelODRMode(LSM_DriverHandle_t * dev, LSM_AccelODR_t * m)
    {
        status_t status = STATUS_OK;
    ...
    status_t LSM_setGyroODRMode(LSM_DriverHandle_t * dev, LSM_GyroODR_t * m)
    {
        status_t status = STATUS_OK;
    ...
    status_t LSM_getGyroODRMode(LSM_DriverHandle_t * dev, LSM_GyroODR_t * m)
    {
        status_t status = STATUS_OK;
    ...
    status_t LSM_set_accel_full_scale(LSM_DriverHandle_t * dev, uint8_t * fs)
    {
        status_t err = STATUS_OK;
        uint8_t f = *fs;
        uint8_t regval = 0;
    ...
    status_t LSM_get_accel_full_scale(LSM_DriverHandle_t * dev, uint8_t * fs)
    {
        status_t status = STATUS_OK;
    ...
    status_t LSM_set_gyro_full_scale(LSM_DriverHandle_t * dev, uint8_t * fs)
    {
        status_t err = STATUS_OK;
        uint8_t f = *fs;
        uint8_t regval = 0;
    ...
    status_t LSM_get_gyro_full_scale(LSM_DriverHandle_t * dev, uint8_t * fs)
    {
        status_t status = STATUS_OK;
    ...
    status_t LSM_set_interrupt_pp_od(LSM_DriverHandle_t * dev, bool *val)
    {
        status_t err = STATUS_OK;
        bool value = *val;
        uint8_t regval = 0;
    ...
    status_t LSM_set_interrupt_active_level(LSM_DriverHandle_t * dev, bool *val)
    {
        status_t err = STATUS_ERR_NOT_SUPPORTED;
        return err;
    }

    status_t LSM_set_spi_mode(LSM_DriverHandle_t * dev, bool *val)
    {
        status_t err = STATUS_ERR_NOT_SUPPORTED;
        return err;
    }

    status_t LSM_set_auto_increment_addr(LSM_DriverHandle_t * dev, bool *val)
    {
        status_t err = STATUS_ERR_NOT_SUPPORTED;
        return err;
    }
    ...
    status_t LSM_set_block_data_update_en(LSM_DriverHandle_t * dev, bool *en)
    {
        bool e = *en;
        bool wr = false;
        uint8_t regval = 0;
    ...
    status_t LSM_get_block_data_update_en(LSM_DriverHandle_t * dev, bool *en)
    {
        status_t status = STATUS_OK;
    ...
    status_t LSM_set_gyro_sleep_state(LSM_DriverHandle_t * dev, LSM_GyroSleepState_t * mode)
    {
        status_t err = STATUS_OK;
        uint8_t m = *mode;
        uint8_t regval = 0;
    ...
    status_t LSM_get_gyro_sleep_state(LSM_DriverHandle_t * dev, LSM_GyroSleepState_t * mode)
    {
        status_t status = STATUS_OK;
    ...
    status_t LSM_set_int2_on_int1_state(LSM_DriverHandle_t * dev, uint8_t * mode)
    {
        status_t err = STATUS_OK;
        uint8_t m = *mode;
        uint8_t regval = 0;
    ...
    status_t LSM_get_int2_on_int1_state(LSM_DriverHandle_t * dev, uint8_t * mode)
    {
        status_t status = STATUS_OK;
    ...
    status_t LSM_set_temperature_pkt_en(LSM_DriverHandle_t * dev, bool *en)
    {
        status_t err = STATUS_OK;
        bool e = *en;
        uint8_t regval = 0;
    ...
    status_t LSM_get_temperature_pkt_en(LSM_DriverHandle_t * dev, bool *en)
    {
        status_t status = STATUS_OK;
    ...
    status_t LSM_set_stop_on_fifo_thresh(LSM_DriverHandle_t * dev, bool *en)
    {
        status_t err = STATUS_OK;
        bool e = *en;
        uint8_t regval = 0;
    ...
    status_t LSM_get_stop_on_fifo_thresh(LSM_DriverHandle_t * dev, bool *en)
    {
        status_t status = STATUS_OK;
    ...
    status_t LSM_set_rounding_mode(LSM_DriverHandle_t * dev, uint8_t * mode)
    {
        return STATUS_ERR_NOT_SUPPORTED;
    }

    status_t LSM_set_gyro_hp_mode(LSM_DriverHandle_t * dev, bool *mode)
    {
        status_t err = STATUS_OK;
        bool e = *mode;
        uint8_t regval = 0;
    ...
    status_t LSM_get_gyro_hp_mode(LSM_DriverHandle_t * dev, bool *mode)
    {
        status_t status = STATUS_OK;
    ...
    status_t LSM_set_gyro_hpfilter_mode(LSM_DriverHandle_t * dev, bool *mode)
    {
        status_t err = STATUS_OK;
        bool e = *mode;
        uint8_t regval = 0;
    ...
    status_t LSM_get_gyro_hpfilter_mode(LSM_DriverHandle_t * dev, bool *mode)
    {
        status_t status = STATUS_OK;
    ...
    status_t LSM_set_gyro_hp_filter_cutoff(
        LSM_DriverHandle_t * dev,
    ...
    status_t LSM_get_gyro_hp_filter_cutoff(
        LSM_DriverHandle_t * dev,
        LSM_GyroHighpassCutoff_t * cutoff)
    {
        status_t status = STATUS_OK;
    ...
    status_t LSM_set_accel_lp2_filter_en(LSM_DriverHandle_t * dev, bool *en)
    {
        status_t err = STATUS_OK;
        bool e = *en;
        uint8_t regval = 0;
    ...
    status_t LSM_get_accel_lp2_filter_en(LSM_DriverHandle_t * dev, bool *en)
    {
        status_t status = STATUS_OK;
    ...
    status_t LSM_set_accel_hp_filter_cutoff(
        LSM_DriverHandle_t * dev,
    ...
    status_t LSM_get_accel_hp_filter_cutoff(
        LSM_DriverHandle_t * dev,
        LSM_HighpassSlopeSettings_t * cutoff)
    {
        status_t status = STATUS_OK;
    ...
    status_t LSM_set_accel_hpfilter_mode(LSM_DriverHandle_t * dev, bool *mode)
    {
        status_t err = STATUS_OK;
        bool e = *mode;
        uint8_t regval = 0;
    ...
    status_t LSM_get_accel_hpfilter_mode(LSM_DriverHandle_t * dev, bool *mode)
    {
        status_t status = STATUS_OK;
    ...
    status_t LSM_set_lowpass_on_6d_mode(LSM_DriverHandle_t * dev, bool *mode)
    {
        status_t err = STATUS_OK;
        bool e = *mode;
        uint8_t regval = 0;
    ...
    status_t LSM_get_lowpass_on_6d_mode(LSM_DriverHandle_t * dev, bool *mode)
    {
        status_t status = STATUS_OK;
    ...
    status_t LSM_set_function_en(LSM_DriverHandle_t * dev, bool *mode)
    {
        status_t err = STATUS_OK;
        bool e = *mode;
        uint8_t regval = 0;
    ...
    status_t LSM_get_function_en(LSM_DriverHandle_t * dev, bool *mode)
    {
        status_t status = STATUS_OK;
    ...
    status_t LSM_sample_latest(LSM_DriverHandle_t * dev)
    {
        status_t status = STATUS_OK;
    ...
    status_t LSM_update_temperature(LSM_DriverHandle_t * dev)
    {
        status_t status = STATUS_OK;
    ...
    status_t LSM_get_temperature(LSM_DriverHandle_t * dev, int16_t * temp)
    {
        *temp = dev->measurements.calibTemperature;
        return STATUS_OK;
    }

    status_t LSM_getGyroX(LSM_DriverHandle_t * dev, float *x)
    {
        status_t status = STATUS_OK;
    ...
    status_t LSM_getGyroY(LSM_DriverHandle_t * dev, float *y)
    {
        status_t status = STATUS_OK;
    ...
    status_t LSM_getGyroZ(LSM_DriverHandle_t * dev, float *z)
    {
        status_t status = STATUS_OK;
    ...
    status_t LSM_getAccelX(LSM_DriverHandle_t * dev, float *x)
    {
        status_t status = STATUS_OK;
    ...
    status_t LSM_getAccelY(LSM_DriverHandle_t * dev, float *y)
    {
        status_t status = STATUS_OK;
    ...
    status_t LSM_getAccelZ(LSM_DriverHandle_t * dev, float *z)
    {
        status_t status = STATUS_OK;
    ...
    status_t LSM_get_fifo_pkt_index(LSM_DriverHandle_t * dev, uint16_t * index)
    {
        status_t err = STATUS_OK;
        uint8_t raw[2] = {0};
    ...
    status_t LSM_set_fifo_step_en(LSM_DriverHandle_t * dev, bool *en)
    {
        status_t err = STATUS_OK;
        bool e = *en;
        uint8_t regval = 0;
    ...
    status_t LSM_get_fifo_step_en(LSM_DriverHandle_t * dev, bool *en)
    {
        status_t err = STATUS_OK;
    ...
    status_t LSM_set_only_msb_data(LSM_DriverHandle_t * dev, bool *en)
    {
        status_t err = STATUS_OK;
        bool e = *en;
        uint8_t regval = 0;
    ...
    status_t LSM_get_only_msb_data(LSM_DriverHandle_t * dev, bool *en)
    {
        status_t err = STATUS_OK;
    ...
    status_t LSM_get_fifo_pkt_count(LSM_DriverHandle_t * dev, uint16_t * count)
    {
        status_t status = STATUS_OK;
    ...
    status_t LSM_set_fifo_step_drdy(LSM_DriverHandle_t * dev, uint8_t * on_step_en)
    {
        status_t err = STATUS_OK;
        bool e = *on_step_en;
        uint8_t regval = 0;
    ...
    status_t LSM_get_fifo_step_drdy(LSM_DriverHandle_t * dev, uint8_t * on_step_en)
    {
        status_t err = STATUS_OK;
    ...
    status_t LSM_set_fifo_step_decim(LSM_DriverHandle_t * dev, uint8_t * dec)
    {
        status_t err = STATUS_OK;
        uint8_t d = *dec;
        uint8_t regval = 0;
    ...
    status_t LSM_get_fifo_step_decim(LSM_DriverHandle_t * dev, uint8_t * dec)
    {
        status_t err = STATUS_OK;
    ...
    status_t LSM_set_fifo_odr(LSM_DriverHandle_t * dev, uint8_t * odr)
    {
        status_t err = STATUS_OK;
        uint8_t o = *odr;
        uint8_t regval = 0;
    ...
    status_t LSM_get_fifo_odr(LSM_DriverHandle_t * dev, uint8_t * odr)
    {
        status_t err = STATUS_OK;
    ...
    status_t LSM_set_fifo_mode(LSM_DriverHandle_t * dev, LSM_FIFOMode_t * m)
    {
    ...
    status_t LSM_get_fifo_mode(LSM_DriverHandle_t * dev, uint8_t * mode)
    {
        status_t status = STATUS_OK;
    ...
    status_t LSM_set_fifo_watermark(LSM_DriverHandle_t * dev, uint16_t * watermark)
    {
        status_t status = STATUS_OK;
        uint16_t val = 0;
        val = *watermark;
    ...
    status_t LSM_get_fifo_watermark(LSM_DriverHandle_t * dev, uint16_t * watermark)
    {
        status_t status = STATUS_OK;
    ...
    status_t LSM_setFIFOpackets(LSM_DriverHandle_t * device, LSM_PktType_t * pT)
    {
        /**
    ...
    status_t LSM_read_fifo_to_cbuffer(LSM_DriverHandle_t * dev)
    {
        status_t err = STATUS_OK;
        ...
        if (dev->cbuff_store_packets) {
            ...
            if (!dev->fifo_pkts.configured) {
                log_error("LSM_Driver", "Error: Fifo packets are not configured");
                err = STATUS_ERR_INVALID_STATE;
            }
            /** not enough bytes for a whole packet **/
            if (bytes_available < packet_len) {
                log_info("LSM_Driver", "No full packets available at this time!");
                err = STATUS_ERR_INVALID_STATE;
            }
            ...
            for (int j = 0; j < full_packets; j++) {
                ...
                if (err) {
                    log_error("LSM_Driver", "Error reading fifo block to cbuffer {%u}", err);
                    break;
                }
            }

            if (!err) {
                log_info("LSM Driver", "Stored %u packets in cbuffer!", full_packets);
            }
        } else {
        ...
        if (!err && samples_available > 0) {
            ...
            err = gcd_i2c_read_address(
                ...
                bytes_available,
                buffer);
            log_info("Dropped %u samples during read\n", samples_available);
        }

        err = LSM_reset_fifo(dev);

        if (err) {
            log_error("LSM_Driver", "Error in fifo read: %u", err);
        }
     ...
     *
     **/
    status_t LSM_readFifoBlock(LSM_DriverHandle_t * dev, uint16_t * num_samples)
    {
        status_t status = STATUS_OK;
        ...
        if (!status && dev->use_cbuffer) {
            log_info("Attempting to store packets in cbuffer...\n");
            ...
            log_error("LSM_Driver", "Error, no buffer configured for fifo!");
        }
    ...
    status_t LSM_set_interrupt_1(LSM_DriverHandle_t * device, uint8_t * int_t)
    {
        status_t status = STATUS_OK;
        ...
        if (device->i1Pin > 0) {
            ...
            log_error("LSM_DRIVER", "Error: Pin not configured!");
            status = STATUS_ERR_NOT_FOUND;
        }
    ...
    status_t LSM_set_interrupt_2(LSM_DriverHandle_t * device, uint8_t * int_t)
    {
        status_t status = STATUS_OK;
        uint8_t regval = 0;
        uint8_t intr = *int_t;
    ...
    status_t LSM_set_function_intr1(LSM_DriverHandle_t * dev, uint8_t * int_t)
    {
        return STATUS_ERR_NOT_SUPPORTED;
    }

    status_t LSM_set_function_intr2(LSM_DriverHandle_t * dev, uint8_t * int_t)
    {
        return STATUS_ERR_NOT_SUPPORTED;
    }
