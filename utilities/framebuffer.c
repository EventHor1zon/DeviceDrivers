...
#define PIXEL_SET_BIT_CARTESIAN(fb, x, y)                                                                       \
    (*(uint8_t                                                                                                  \
           *)(fb->frame_start + (x % fb->frame_width) + (((uint16_t)(y / BITS_PER_BYTE)) * fb->frame_width)) |= \
     (1 << (y % BITS_PER_BYTE)))

#define PIXEL_CLR_BIT_CARTESIAN(fb, x, y)                                                                       \
    (*(uint8_t                                                                                                  \
           *)(fb->frame_start + (x % fb->frame_width) + (((uint16_t)(y / BITS_PER_BYTE)) * fb->frame_width)) &= \
     ~(1 << (y % BITS_PER_BYTE)))

#define PIXEL_PTR_FROM_CARTESIAN(fb, x, y)                                                         \
    (uint8_t *)fb->frame_start + ((x % frame->width) * fb->pixel_size)                             \
        + ((y * frame->width) * fb->pixel_size)

#define COORDS_OUTSIDE_FRAME_BOUNDS(fb, x, y) ((x >= fb->frame_width) || (y >= fb->frame_height))

/** TODO: Fix these */
#define PIXEL_SET_BIT(fb, index, bit)                                                              \
    (*(void *)(fb->frame_start + (index * fb->pixel_size)) |= (1 << bit))
#define PIXEL_CLR_BIT(fb, index, bit)                                                              \
    (*(void *)(fb->frame_start + (index * fb->pixel_size)) &= ~(1 << bit))
...
status_t framebuffer_init(FB_h fb, framebuff_init_t *init)
{
    ...
    {
        log_error(FB_TAG, "Error, invalid frame dimensions");
    ...
    if (!err) {
        ...
        switch (init->pixel_size) {
            ...
            case PIXEL_TYPE_8BIT: fb->frame_len = fb->frame_height * fb->frame_width; break;
            case PIXEL_TYPE_16BIT: fb->frame_len = fb->frame_height * fb->frame_width * 2; break;
            case PIXEL_TYPE_24BIT: fb->frame_len = fb->frame_height * fb->frame_width * 3; break;
            case PIXEL_TYPE_32BIT: fb->frame_len = fb->frame_height * fb->frame_width * 4; break;
            default:
                err = STATUS_ERR_INVALID_ARG;
                log_error(FB_TAG, "Error, invalid pixel size!");
                break;
        }
    ...
    if (!err) {
        ...
        if (fb->frame_start == NULL) {
            ...
            log_error(FB_TAG, "Error, unable to assign frame heap memory!");
        } else {
            frame_assigned = true;
    ...
    if (err) {
        log_error(FB_TAG, "Failed to initialise framebuffer [%u]", err);
...
#ifdef DEBUG_MODE
            log_info(FB_TAG, "Dropping duplicate set");
#endif /* DEBUG_MODE */
        } else {
#ifdef DEBUG_MODE
            log_info(
                "Iter %lu: p->x (%d) p->y (%d)  --  np->x (%d) np->y (%d)\n",
                i,
                p.x,
                p.y,
                np.x,
...
status_t framebuffer_draw_vertical_ysteps(FB_h fb, coord_t *start, coord_t *end)
{
    status_t err = STATUS_OK;
    ...
    {
        log_error(FB_TAG, "(%u %u) (%u %u) out of bounds", start->x, start->y, end->x, end->y);
        err = STATUS_ERR_INVALID_ARG;
...
status_t framebuffer_draw_horizontal_xsteps(FB_h fb, coord_t *start, coord_t *end)
{
    status_t err = STATUS_OK;
        ...
        || COORDS_OUTSIDE_FRAME_BOUNDS(fb, end->x, end->y))
    {
        ...
        /** line of len 0 or -ve line **/
        log_error(FB_TAG, "(%u %u) invalid line length", start->x, end->x);
        err = STATUS_ERR_INVALID_SIZE;
...
status_t framebuffer_draw_line_xsteps(FB_h fb, coord_t *start, coord_t *end)
{
    status_t err = STATUS_OK;
    float slope = 0.0;
    uint16_t diff = 0;
...
status_t framebuffer_clear_buffer(FB_h fb)
{
    memset(fb->frame_start, 0, sizeof(uint8_t) * fb->frame_len);
    return STATUS_OK;
}
